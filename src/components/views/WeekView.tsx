...
import { format, isToday, isTomorrow, isPast, addDays, startOfWeek, endOfWeek, isWeekend, parseISO, getDay } from "date-fns"
import { 
  Calendar, 
  Clock, 
  CheckCircle2, 
...
export function WeekView({ onTaskEdit, onTaskView, onAddTask }: WeekViewProps) {
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      console.error("Failed to load week's tasks:", error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  // Group tasks for the week view
  const groupedTasks = React.useMemo(() => {
    const groups: { id: string; title: string; tasks: TaskWithRelations[]; date?: Date }[] = []
    const now = new Date()
    ...
    const weekendTasks: TaskWithRelations[] = []
    const weekTasks: { date: Date; tasks: TaskWithRelations[] }[] = []
    ...
    tasks.forEach((task) => {
      ...
      } else if (isWeekend(taskDate)) {
        weekendTasks.push(task)
      } else if (taskDate >= weekStart && taskDate <= weekEnd) {
        // Add to day-specific group
        const existingDay = weekTasks.find(w => format(w.date, "yyyy-MM-dd") === taskDateStr)
        if (existingDay) {
          existingDay.tasks.push(task)
        } else {
          weekTasks.push({ date: taskDate, tasks: [task] })
        }
    ...
    // Sort week tasks by date
    weekTasks.sort((a, b) => a.date.getTime() - b.date.getTime())
    ...
    // Add day-specific groups for this week (excluding today and tomorrow)
    const dayNames = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    weekTasks
      .filter(w => {
        const str = format(w.date, "yyyy-MM-dd")
        return str !== today && str !== tomorrow
      })
      .forEach(weekTask => {
        groups.push({
          id: format(weekTask.date, "yyyy-MM-dd"),
          title: format(weekTask.date, "EEEE"),
    ...
    if (weekendTasks.length > 0) {
      groups.push({ id: "weekend", title: "This Weekend", tasks: weekendTasks })
    }
  ...
  if (tasks.length === 0) {
    return (
      <EmptyWeekState onAddTask={onAddTask} />
    )
  }
        ...
        {groupedTasks.map((group) => (
          ...
          >
            <h3 className={cn(
              ...
              {group.id === "overdue" && (
                <Button size="sm" variant="ghost" className="ml-auto text-xs h-6">
                  View all
                </Button>
              )}
            </h3>
...
// Empty State
function EmptyWeekState({ onAddTask }: { onAddTask?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="p-4 rounded-full bg-muted mb-4">
        <CalendarDays className="h-8 w-8 text-muted-foreground" />
      </div>
      <h3 className="text-xl font-semibold mb-2">No tasks for the next 7 days</h3>
      <p className="text-muted-foreground mb-6 max-w-sm">
        Your schedule is clear for the next week. Add tasks to plan ahead.
      </p>
      <Button onClick={onAddTask}>
        <Plus className="mr-2 h-4 w-4" />
        Add a task
