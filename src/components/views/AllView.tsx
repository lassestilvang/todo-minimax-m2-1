...
export function AllView({ onTaskEdit, onTaskView, onAddTask }: AllViewProps) {
  ...
  const groupedTasks = React.useMemo(() => {
    ...
    tasks.forEach((task) => {
      // Always include completed tasks in "All" view
    ...
    if (showCompletedTasks && completedTasks.length > 0) {
      groups.push({ id: "completed", title: "Completed", tasks: completedTasks })
    }
  ...
  if (tasks.length === 0) {
    return (
      <EmptyAllState onAddTask={onAddTask} />
    )
  }
        ...
        {groupedTasks.map((group) => (
          ...
          >
            <h3 className={cn(
              ...
              group.id === "completed" ? "text-muted-foreground" : "text-muted-foreground"
            )}>
              {group.id === "completed" ? (
            ...
            <div className="space-y-3">
              {group.tasks.map((task) => (
                <TaskCard
                  ...
                  isCompleted={group.id === "completed"}
                />
              ))}
            </div>
          </motion.div>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {task.labels?.map((label) => (
              <Badge 
                ...
                variant="secondary"
                className="text-xs"
                style={{ backgroundColor: label.color ? label.color + "20" : undefined, color: label.color || undefined }}
              >
                {label.name}
              </Badge>
            ))}
...
// Empty State
function EmptyAllState({ onAddTask }: { onAddTask?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-muted-foreground mb-6 max-w-sm">
        Get started by creating your first task. All your tasks will appear here.
      </p>
      <Button onClick={onAddTask}>
        <Plus className="mr-2 h-4 w-4" />
        Add your first task
