...
export function TodayView({ onTaskEdit, onTaskView, onAddTask }: TodayViewProps) {
  ...
  const groupedTasks = React.useMemo(() => {
    ...
    tasks.forEach((task) => {
      ...
      } else if (format(taskDate, "yyyy-MM-dd") === today) {
        ...
        {groupedTasks.map((group) => (
          ...
          >
            {groupedTasks.length > 1 && (
              <h3 className={cn(
                "text-sm font-medium uppercase tracking-wider mt-6 mb-2",
...
// Task Card Component for custom Today view rendering
function TaskCard({
  task,
  onEdit,
  onViewDetails,
  ...
  const { toggleShowCompletedTasks } = useUIStore()
...
function EmptyTodayState({ onAddTask }: { onAddTask?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-muted-foreground mb-6 max-w-sm">
        You have no tasks scheduled for today. Enjoy your free time or add a new task to get started.
      </p>
      <Button onClick={onAddTask}>
        <Plus className="mr-2 h-4 w-4" />
        Add your first task
